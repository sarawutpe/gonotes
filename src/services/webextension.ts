...
import { GroupNote, Note, GroupNoteData, STORAGE_KEY_GROUP_NOTE, STORAGE_KEY_NOTE } from '@services/webextension.type';
...
export const chromeGetGroupNoteList = async (): Promise<GroupNoteData[] | []> => {
  try {
    const groupNoteContext = await chrome.storage.sync.get(STORAGE_KEY_GROUP_NOTE);
    const noteContext = await chrome.storage.sync.get(STORAGE_KEY_NOTE);
    const groupNoteList: GroupNote[] = groupNoteContext[STORAGE_KEY_GROUP_NOTE] || [];
    const noteList: Note[] = noteContext[STORAGE_KEY_NOTE] || [];

    // Default folder
    groupNoteList.push({ id: '', name: 'Others', createdDate: '', updatedDate: '' });
...
export const chromeAddNote = debounce(async (content: string, groupId = ''): Promise<Note | null> => {
  try {
