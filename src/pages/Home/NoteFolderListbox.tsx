...
import { chromeGetGroupNoteList, chromeNoteUpdateGroupId } from '@services/webextension';
...
const NoteFolderListbox: React.FC<NoteFolderListboxProps> = ({ open, onClose }) => {
  ...
  const [selectedGroupNote, setSelectedGroupNote] = useState<GroupNote>({ id: '', name: 'Others', createdDate: '', updatedDate: '' });
                            ...
                            <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
                              {groupNoteList.map((groupNote) => (
                                ...
                                >
                                  {({ selected }) => (
                                    <>
                                      <span className={`block truncate ${selected ? 'font-medium' : 'font-normal'}`}>{groupNote.name}</span>
                                      {selected ? (
